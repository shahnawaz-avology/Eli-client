...
class MenuDrawer extends HTMLElement {
  ...
  onSummaryClick(event) {
    ...
    // Check if the click target is within the span element
    const spanElement = summaryElement.querySelector("span");
    const isSpanClicked = spanElement.contains(target);

    // If the click is on the span tag, navigate to the specified URL
    // if (isSpanClicked) {
    //     const url = spanElement.getAttribute('onclick').replace('window.location.href=', '').replace(';', '').trim();
    //     window.location.href = url;
    //     return;
    // }

    // If the click target is not within the summary element, don't proceed
    if (target !== summaryElement && !summaryElement.contains(target)) return;
